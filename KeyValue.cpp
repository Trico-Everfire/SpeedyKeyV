...
KeyValueRoot* KeyValueRoot::Create(const char* str)
{
	KeyValueRoot* root = new KeyValueRoot(str);
	if (root->IsValid())
		return root;
	delete root;
	return nullptr;
...
KeyValue& KeyValue::operator[](const char* key) const
{
	return Get(key);
}

KeyValue& KeyValue::operator[](size_t index) const
{
	return Get(index);
}

KeyValue& KeyValue::Get(const char* key) const
{
...
KeyValue& KeyValue::Get(size_t index) const
{
